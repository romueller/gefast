...
//    void SegmentFilter::filterForward(AmpliconCollection& ac, RotatingBuffers<Candidate>& cands, const lenSeqs_t t, const lenSeqs_t k) {
//
//        RollingIndices<InvertedIndex> indices(t + 1, t + k, true);
//
//        std::vector<numSeqs_t> candIntIds;
...
//
//        lenSeqs_t seqLen = 0;
//        numSeqs_t curIntId = 0;
//
//        for (auto amplIter = ac.begin(); amplIter != ac.end(); amplIter++, curIntId++) { // for every amplicon in the collection...
//
//            // on reaching new length group, open new inverted indices and discard those no longer needed ...
...
//                    for (auto substrPos = subs.first; substrPos <= subs.last; substrPos++) {
//
//                        candIntIds = inv.getLabelsOf(std::string(amplIter->seq, substrPos, subs.len));
//
//                        for (auto candIter = candIntIds.begin(); candIter != candIntIds.end(); candIter++) {
//
//                            candCnts[*candIter]++;
...
//            //index sequence
//            for (lenSeqs_t i = 0; i < t + k; i++) {
//                indices.getIndex(seqLen,i).add(amplIter->seq.substr(segments[i].first, segments[i].second), curIntId);
//            }
//
//        }
//
//    }
//    void SegmentFilter::filterBackward(AmpliconCollection& ac, RotatingBuffers<Candidate>& cands, const lenSeqs_t t, const lenSeqs_t k) {
//
//        RollingIndices<InvertedIndex> indices(t + 1, t + k, false);
//
//        std::vector<numSeqs_t> candIntIds;
...
//
//        lenSeqs_t seqLen = 0;
//        numSeqs_t curIntId = ac.size() - 1; //<-- change from 0
//
//        for (auto amplIter = ac.rbegin(); amplIter != ac.rend(); amplIter++, curIntId--) { // for every amplicon in the collection... //<-- change from begin() / end() and curIntId++
//
//            // on reaching new length group, open new inverted indices and discard those no longer needed ...
//            if (amplIter->seq.length() != seqLen) {
//
...
//
//    }
//    void SegmentFilter::filterForwardBackward(AmpliconCollection& ac, RotatingBuffers<Candidate>& cands, const lenSeqs_t t, const lenSeqs_t k) {
//
//        RollingIndices<InvertedIndex> indices(t + 1, t + k, true);
//
//        std::vector<numSeqs_t> candIntIds;
...
//            // compute actual segments of the current amplicon here once (used in pipelined filtering and indexing steps)
//            for (auto i = 0; i < t + k; i++) {
//                segmentStrs[i] = amplIter->seq.substr(segments[i].first, segments[i].second);
//            }
//
//            for (lenSeqs_t len = (seqLen > t) * (seqLen - t); len <= seqLen; len++) { //... consider already indexed seqs with a feasible length...
//
...
//
//    }
//    void SegmentFilter::filterBackwardForward(AmpliconCollection& ac, RotatingBuffers<Candidate>& cands, const lenSeqs_t t, const lenSeqs_t k) {
//
//        RollingIndices<InvertedIndex> indices(t + 1, t + k, false);
//
//        std::vector<numSeqs_t> candIntIds;
