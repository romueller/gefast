...
#ifndef K2TREES_PARTITIONEDVARIABLEK2TREE_HPP
#define K2TREES_PARTITIONEDVARIABLEK2TREE_HPP
...
#include "Utility.hpp"
#include "NaiveK2Tree.hpp"
#include "../../modes/SpaceLevenshteinMode.hpp"

namespace GeFaST {

    /*
     * Uneven rectangular implementation of K2Tree that uses NaiveK2Tree instances
     ...
     * of the whole relation matrix into several RectangularK2Tree / NaiveK2Tree instances.
     ...
     *
     * Refined class UnevenKrKcOrMiniTree<bool> (https://github.com/romueller/k2trees/blob/master/StaticUnevenRectangularOrMiniTree.hpp).
     */
    template<typename B, typename R, typename I>
    class PartitionedVariableK2Tree : public virtual K2Tree<bool, I> {
    ...
    public:
         ...
         */
        PartitionedVariableK2Tree(positions_t& pairs, const size_t num_rows, const size_t num_cols, const size_t kr, const size_t kc, const size_t mb, const bool perm_rank) {
            ...
            if (hc_ > hr_) {
                ...
                partitions_ = new K2Tree<value_t>*[num_partitions_];
                ...
                for (size_t i = 0; i < num_partitions_; i++) {
                    if (intervals[i].second - intervals[i].first > mb) {
                        ...
                        partitions_[i] = new NaiveK2Tree<index_t>(pairs, 0, i * partition_size_, num_rows_, partition_size_, intervals[i].first, intervals[i].second, kr_, kc_);
                    }
                }
                ...
                for (size_t j = 0; j < num_partitions_; j++) {
                    if (intervals[j].second - intervals[j].first > mb) {
                        ...
                        partitions_[j] = new NaiveK2Tree<index_t>(pairs, j * partition_size_, 0, partition_size_, num_cols_, intervals[j].first, intervals[j].second, kr_, kc_);
                    }
                }
        ...
        PartitionedVariableK2Tree(positions_t& pairs, const size_t kr, const size_t kc, const size_t mb, const bool perm_rank) : PartitionedVariableK2Tree(pairs, 0, 0, kr, kc, mb, perm_rank) {
            // nothing else to do
        }

        PartitionedVariableK2Tree(positions_t& pairs, const K2TreeParameters& config) : PartitionedVariableK2Tree(pairs, 0, 0, config.kr, config.kc, config.mb, config.deep) {
            // nothing else to do
        }
        ...
        PartitionedVariableK2Tree(const PartitionedVariableK2Tree& other) { // copy constructor
            ...
            partitions_ = new K2Tree<value_t, index_t>*[num_partitions_];
            for (size_t k = 0; k < num_partitions_; k++) {
        ...
        PartitionedVariableK2Tree(PartitionedVariableK2Tree&& other) noexcept { // move constructor
        ...
        PartitionedVariableK2Tree& operator=(const PartitionedVariableK2Tree& other) { // copy assignment operator
        ...
        PartitionedVariableK2Tree& operator=(PartitionedVariableK2Tree&& other) noexcept { // move assignment operator
        ...
        PartitionedVariableK2Tree* clone() const override {
            return new PartitionedVariableK2Tree(*this);
        }
        ...
        positions_t get_range(index_t i1, index_t i2, index_t j1, index_t j2) override {
            ...
            if (upper_left.partition == lower_right.partition) {
                ...
                if (p != nullptr) {
                    ...
                    elements = p->get_range(upper_left.row, lower_right.row, upper_left.col, lower_right.col);
            ...
            // flatten results
            size_t to_reserve = elements.size();
            for (auto& v : tmp) {
                to_reserve += v.size();
            }
            elements.reserve(to_reserve);
        ...
        size_t size_in_bytes() const override {
            ...
            size_t res = sizeof(PartitionedVariableK2Tree<B, R, I>)  // PartitionedVariableK2Tree itself
                    + num_partitions_ * sizeof(K2Tree<bool, I>*); // partitions_ (pointers)
            for (size_t i = 0; i < num_partitions_; i++) {
        ...
        // note: can "invalidate" the data structure (contains_link() probably won't work correctly afterwards)
        void set_null_shallow(index_t i, index_t j) override {
    ...
    private:
        ...
        K2Tree<value_t>** partitions_; // representations of the partitions / submatrices
...
#endif //K2TREES_PARTITIONEDVARIABLEK2TREE_HPP
