...
namespace GeFaST {
     ...
     * For example, when adding entries the identifiers are expected to be monotically increasing.
     */
    class SegmentRelations {

        typedef std::unordered_map<numSeqs_t, numSeqs_t> length_map_type;
        typedef std::unordered_map<StringIteratorPair, std::vector<numSeqs_t>, hashStringIteratorPair, equalStringIteratorPair> sip_map_type;
    ...
    public:
        ...
        void add_counts(const numSeqs_t len, const numSeqs_t seg, const StringIteratorPair& sip, std::vector<numSeqs_t>& cand_cnts);
        void add_counts_checked(const numSeqs_t len, const numSeqs_t seg, const StringIteratorPair& sip, std::vector<numSeqs_t>& cand_cnts);

        // "count" the amplicons of the given length whose seg-th segment is equal to the given substring
        // or the num_neigh next substrings generated by shifting to the right,
        // appends the ids of the found amplicons to the given vector
        // IMPORTANT: The existence of the length must be checked beforehand (or use add_counts_checked).
        void add_neighboured_counts(const numSeqs_t len, const numSeqs_t seg, StringIteratorPair sip,
                                    numSeqs_t num_neigh, std::vector<numSeqs_t>& cand_cnts);
        void add_neighboured_counts_checked(const numSeqs_t len, const numSeqs_t seg, StringIteratorPair sip,
                                            numSeqs_t num_neigh, std::vector<numSeqs_t>& cand_cnts);

        // inserts the given segment in the relation for the given length and segment number
        void record_segment(const numSeqs_t len, const numSeqs_t seg, const StringIteratorPair& sip, const numSeqs_t ampl_id);
    ...
    private:
        // array of mappings from StringIteratorPair to vector of ids,
